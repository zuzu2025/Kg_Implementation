...
This engine is separate from the evaluation harness, which compares stage 6
ontology design facts against stage 7 validated triples.
...
Three retrieval channels, merged:
...
class RagEngine:
    ...
    def query(self, query_text, top_k_sentences=5, max_kg_facts_per_entity=5,
        ...
        for idx in candidate_idxs:
            ...
            final_score = bm25_score * (1 + GRAPH_BOOST_WEIGHT * graph_boost)
        ...
        for s in matched_sentences:
            entities_seen.update(s["entities"])
...
def format_result(result):
    ...
    for i, s in enumerate(result["matched_sentences"], 1):
        ...
        lines.append(f"{i}. (score {s['score']} = bm25 {s['bm25_score']}{boost_note}) {s['text']}")
        if s["entities"]:
