"""
stage12_rag_engine.py (v4)

A local retrieval engine over the contract NER + knowledge-graph pipeline
(stages 1-11), now with an LLM SYNTHESIS layer on top (Groq API).

v4 change from v3: v1-v3 were pure extractive retrieval -- they rank and
DISPLAY evidence, but nothing ever judges whether that evidence actually
answers the question. That's the "just giving me what it knows" problem.
v4 adds one more step: the top retrieved evidence (matched sentences +
validated KG facts + unverified backfill + reasoning paths -- exactly
what v3 already computes, nothing new retrieved) gets handed to Groq
with an instruction to answer the question directly using ONLY that
evidence, and to explicitly say so if the evidence doesn't answer it --
rather than describing what's there. Retrieval itself is UNCHANGED.
...
If GROQ_API_KEY isn't set, or the API call fails, this falls back to
v3's behavior: raw retrieval output only, no synthesized answer, with a
printed note -- same graceful-degradation pattern as the embedding
channel in v3.
...
Usage:
    python3 rag_engine_v4.py                             # interactive REPL
    python3 rag_engine_v4.py "who is the distributor"    # one-shot query

Or import and call answer() programmatically:
    from rag_engine_v4 import RagEngine
    engine = RagEngine()
    result = engine.answer("governing law of the agreement")
...
# so you don't have to re-type `$env:GROQ_API_KEY = ...` every session.
# This file lives ONLY on your machine -- never paste its contents into a
# chat, and add it to .gitignore if this folder is ever a git repo.
def _load_dotenv(path=".env"):
    if not os.path.exists(path):
        print(f"[.env] No {path} found in the current folder ({os.getcwd()}) -- "
              f"relying on already-set environment variables, if any.")
        return
    loaded_keys = []
    # utf-8-sig strips a leading BOM automatically if present -- Windows
    # Notepad saves "UTF-8 with BOM" by default, which otherwise silently
    # corrupts the first line's key name (e.g. GROQ_API_KEY stops
    # matching because of an invisible character in front of it).
    ...
    print(f"[.env] Loaded from {os.path.abspath(path)}: {', '.join(loaded_keys) or '(no valid lines found)'}")
...
CACHE_PATH = os.path.join(DATA_DIR, "rag_index_cache_v4.pkl")  # bumped from v3: cache now also
# includes the suffix-stripped core-entity-name index (see strip_legal_suffix / entity recall fix)
...
# --- Groq synthesis layer (v4, switched from Gemini) ---
...
# --- Entity recall fix ---
# Entities are stored under their full NER span, e.g. "Electric City Corp."
# or "Beta Distribution, LLC" -- but people naturally ask about "Electric
# City" or "Beta Distribution" without the legal suffix. The ORIGINAL
# matcher required the full stored name to appear verbatim in the query,
# so any query phrased without the suffix recognized ZERO entities --
# which silently starves KG facts, multi-hop paths, and graph boosting,
# and is why "answer IS in the contracts" queries were coming back as
# "not enough information": the evidence never made it into the prompt.
# This adds a SECOND matching pass on suffix-stripped "core" names, only
# over text the exact pass didn't already claim, so precision on exact
# matches is unaffected and this only ever ADDS recall.
LEGAL_SUFFIX_RE = re.compile(
    r",?\s*\b(incorporated|corporation|corp|company|co|"
    r"limited liability company|llc|l\.l\.c|limited|ltd|"
    r"l\.p|lp|llp|plc|inc)\.?\s*$",
    re.IGNORECASE,
)
MIN_CORE_ENTITY_LEN = 4  # don't core-match names so short they'd overmatch (e.g. "Co")


def strip_legal_suffix(name: str) -> str:
    """Iteratively strip trailing legal-entity suffixes (handles cases like
    'X Holdings, LLC' -> 'X Holdings, LLC' -> 'X Holdings' -> stop, since
    'Holdings' isn't a suffix) so 'Beta Distribution, LLC' and 'Beta
    Distribution' resolve to the same core name."""
    prev = None
    core = name.strip()
    while prev != core:
        prev = core
        core = LEGAL_SUFFIX_RE.sub("", core).strip().strip(",.")
    return core
...
class RagEngine:
    ...
    def _build(self):
        ...
        # --- 6. Core-name index (entity recall fix): suffix-stripped name
        # -> canonical full name(s). Built once here, used at query time by
        # _match_entities_in_text's second pass. A core can map to more than
        # one full name (e.g. two different "X Distribution" entities with
        # different suffixes across different contracts) -- we keep every
        # canonical form and let the caller's existing coverage/scoring
        # logic sort out duplicates rather than silently picking one.
        core_to_full = defaultdict(set)
        for name in all_known_entity_names:
            core = strip_legal_suffix(name)
            if core and core.lower() != name.lower() and len(core) >= MIN_CORE_ENTITY_LEN:
                core_to_full[core.lower()].add(name)
        self.core_entity_names_sorted = sorted(core_to_full.keys(), key=len, reverse=True)
        self.core_to_full_entity = dict(core_to_full)
    ...
    def _save_to_cache(self):
        with open(CACHE_PATH, "wb") as f:
            pickle.dump({
                ...
                "core_entity_names_sorted": self.core_entity_names_sorted,
                "core_to_full_entity": self.core_to_full_entity,
    ...
    def _match_entities_in_text(self, text):
        ...
        Pass 1 (exact): full stored name must appear verbatim. Highest
        precision, unchanged from v3/v4.

        Pass 2 (core, entity-recall fix): only runs over text NOT already
        claimed by pass 1. Matches the suffix-stripped core of a stored
        name (e.g. "Electric City" for stored "Electric City Corp.") with
        word boundaries, so a query that drops the legal suffix -- which is
        the normal way people talk -- still resolves to the canonical
        entity used as the key into node_types / relations_by_node. This
        only ever adds recall; it never overrides an exact-pass match."""
        text_lower = text.lower()
        found = []
        found_set = set()
        covered = [False] * len(text_lower)

        # --- Pass 1: exact full-name match (original behavior) ---
        ...
        # --- Pass 2: suffix-stripped core match, word-boundary, uncovered
        # text only. Longest-core-first for the same reason as pass 1. ---
        for core_lower in self.core_entity_names_sorted:
            pattern = r"\b" + re.escape(core_lower) + r"\b"
            for m in re.finditer(pattern, text_lower):
                start, end = m.start(), m.end()
                if any(covered[start:end]):
                    continue
                for full_name in self.core_to_full_entity.get(core_lower, ()):
                    if full_name not in found_set:
                        found.append(full_name)
                        found_set.add(full_name)
                for i in range(start, end):
                    if i < len(covered):
                        covered[i] = True
                break  # one match per core is enough; move to the next core
    ...
    # ------------------------------------------------------------------
    # v4: LLM synthesis layer -- sits ON TOP of retrieval above, which is
    # completely unchanged. Retrieval decides WHAT evidence to show;
    # synthesis decides whether that evidence actually ANSWERS the
    # question, and says so either way instead of just displaying facts.
    # ------------------------------------------------------------------
    def answer(self, query_text, top_k_sentences=5, max_kg_facts_per_entity=5,
               use_graph_reranking=True):
        result = self.query(
            query_text,
            top_k_sentences=top_k_sentences,
            max_kg_facts_per_entity=max_kg_facts_per_entity,
            use_graph_reranking=use_graph_reranking,
        )
        result["synthesized_answer"] = synthesize_answer(result)
        return result


def _build_synthesis_prompt(result):
    """Turns the retrieval output (matched sentences, validated KG facts,
    unverified backfill, reasoning paths) into a plain-text evidence
    block for the LLM -- exactly what v3 already computes, nothing new
    retrieved here. Validated and unverified evidence are kept in
    clearly separate, labeled sections, same principle as the rest of
    this pipeline."""
    ...
    if result["matched_sentences"]:
        lines.append("Matching clauses from the contracts:")
        for s in result["matched_sentences"]:
            lines.append(f"- \"{s['text']}\"")
        lines.append("")
    if result["kg_facts"]:
        lines.append("Validated knowledge-graph facts:")
        for ent, info in result["kg_facts"].items():
            for fact in info["facts"]:
                lines.append(f"- {fact['source']} {fact['relation']} {fact['target']} "
                              f"(confidence {fact['confidence']})")
        lines.append("")
    if result.get("reasoning_paths"):
        lines.append("Multi-hop reasoning chains:")
        for p in result["reasoning_paths"]:
            chain = " -> ".join(f"{e['source']} {e['relation']} {e['target']}"
                                 for e in p["path"])
            lines.append(f"- {p['source']} to {p['target']}: {chain}")
        lines.append("")
    if not (result["matched_sentences"] or result["kg_facts"] or result.get("reasoning_paths")):
        lines.append("(none found)")
        lines.append("")

    if result.get("unverified_kg_facts"):
        lines.append("UNVERIFIED EVIDENCE (pre-validation, stage 6 -- may be inaccurate):")
    ...
    if result.get("query_entities_missing_from_kg"):
        lines.append(
            "Note: recognized in the contracts but has no facts in the "
            "validated graph: " + ", ".join(result["query_entities_missing_from_kg"])
        )
        lines.append("")

    lines.append(
        "Instructions: Answer the question directly using ONLY the evidence "
        "above. Do not list out the evidence -- synthesize a real answer. "
        "Prefer validated evidence; you may use unverified evidence only if "
        "no validated evidence addresses the question, and if you do, say "
        "explicitly that the answer relies on unverified information. If the "
        "evidence above does not actually answer the question, say plainly "
        "that you don't have enough information to answer it -- do not "
        "describe what evidence exists instead of answering."
    )
    return "\n".join(lines)


def synthesize_answer(result):
    """Calls Groq to turn retrieval output into a real answer (or an
    honest 'not enough information'). Falls back to None (with a printed
    note) if GROQ_API_KEY isn't set or the call fails -- the caller can
    still use the raw retrieval fields exactly as in v3."""
    api_key = os.environ.get("GROQ_API_KEY")
    if not api_key:
        print("[synthesis disabled] GROQ_API_KEY not set -- showing raw retrieval only "
              "(same as v3). Get a key at https://console.groq.com/keys")
        return None

    try:
        import requests
    except ImportError:
        print("[synthesis disabled] `requests` not installed -- "
    ...
    try:
        ...
        if not resp.ok:
            # surface Groq's actual error body -- a bare status code hides
            # the real reason (bad key, deprecated model, rate limit, etc.)
            print(f"[synthesis failed] Groq API returned {resp.status_code}: {resp.text}")
            return None
        data = resp.json()
        choices = data.get("choices", [])
        if not choices:
            print(f"[synthesis failed] Groq returned no choices: {data}")
            return None
        text = (choices[0].get("message", {}) or {}).get("content", "").strip()
        return text or None
    except Exception as e:
        print(f"[synthesis failed] Groq API call error: {e} -- showing raw retrieval only.")
        return None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        q = " ".join(sys.argv[1:])
        print(format_result(engine.answer(q)))
    else:
